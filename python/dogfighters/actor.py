...
class Actor:
    ...
    def generate_proposal(self, problem: str, draft: Optional[str] = None) -> str:
        ...
        if self.debug_mode:
            print(f"Generating proposal by {self.name}...")
        proposal = self.llm_client.generate_text(prompt, max_tokens=MAX_TOKENS)
        if self.debug_mode:
            print(f"Proposal generated by {self.name}.")
    ...
    def vote_on_draft(
        ...
        if self.debug_mode:
            print(f"Voting on draft by {self.name}...")
        response = self.llm_client.generate_text(prompt, max_tokens=MAX_TOKENS)
        if self.debug_mode:
            print(f"Voting on draft by {self.name} completed.")
